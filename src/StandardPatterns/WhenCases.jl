...
function split_case_and_block(stmts, first_bindings, first_source)
    ...
    for stmt in stmts
        @switch stmt begin
            @case :(@when $source begin
                $(bindings...)
            end) || Q[@when $source $elt] && let bindings = [elt]
            end
...
macro otherwise()
    throw(
        SyntaxError(
            "@otherwise is only used inside @when block, as a token to indicate default case.",
        ),
    )
end
...
function gen_when(let_expr, source::LineNumberNode, mod::Module)
    @switch let_expr begin
        @case Expr(
            :let,
            Expr(:block, bindings...) || a && let bindings = [a]
            end,
            Expr(:block, stmts...) || b && let stmts = [b]
            end,
        )
        sources_cases_blocks = split_case_and_block(stmts, bindings, source)
        return foldr(
            sources_cases_blocks,
            init = :nothing,
        ) do (source, bindings, block), last_block
        ...
        throw(
            SyntaxError(
                "Expected a let expression, got a `$short_msg` at $(string(source)).",
            ),
        )
    end
end
...
macro when(assignment, ret)
    @match assignment begin
        :($_ = $_) => let let_expr = Expr(:let, Expr(:block, assignment), ret)
            res = gen_when(let_expr, __source__, __module__)
            res = init_cfg(res)
            esc(res)
        end
